...
function buildFinanceSectionContent(section: Section, lang: string): string {
  ...
  if (section.type === 'summary') {
    return `<p class="text-sm leading-relaxed text-slate-600">${md((c as SummaryContent).text)}</p>`;
  }
  ...
  if (section.type === 'work_experience') {
    return `<div class="space-y-4">${((c as WorkExperienceContent).items || []).map((it: any) => `<div>
      ...
      ${it.description ? `<p class="mt-1 text-sm text-slate-600">${md(it.description)}</p>` : ''}
  ...
  if (section.type === 'custom') {
    return `<div class="space-y-3">${((c as CustomContent).items || []).map((it: any) => `<div>
      ...
      ${it.description ? `<p class="mt-0.5 text-sm text-slate-600">${md(it.description)}</p>` : ''}
    </div>`).join('')}</div>`;
  }
  ...
  if (c.items) {
    return `<div class="space-y-2">${c.items.map((it: any) => `<div><span class="text-sm font-medium" style="color:${SLATE_800}">${esc(it.name || it.title || it.language)}</span>${it.description ? `<p class="text-sm text-slate-600">${md(it.description)}</p>` : ''}</div>`).join('')}</div>`;
  }
