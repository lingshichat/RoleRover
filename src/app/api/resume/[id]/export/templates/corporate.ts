...
function buildCorporateSectionContent(section: Section, lang: string): string {
  ...
  if (c.items) {
    return `<div class="space-y-2">${c.items.map((it: any) => `<div><span class="text-sm font-medium" style="color:${NAVY}">${esc(it.name || it.title || it.language)}</span>${it.description ? `<p class="text-sm text-slate-600">${md(it.description)}</p>` : ''}</div>`).join('')}</div>`;
  }
