...
function buildProfessionalSectionContent(section: Section, lang: string = 'en'): string {
  ...
  const BLUE = '#1e3a5f';
  if (section.type === 'summary') return `<p class="text-sm leading-relaxed text-zinc-600" style="font-family:Georgia,serif">${md((c as SummaryContent).text)}</p>`;
  ...
  if (c.items) {
    return `<div class="space-y-2">${c.items.map((it: any) => `<div><span class="text-sm font-medium" style="color:${BLUE}">${esc(it.name || it.title || it.language)}</span>${it.description ? `<p class="text-sm text-zinc-600">${md(it.description)}</p>` : ''}</div>`).join('')}</div>`;
  }
  return '';
}
