...
function buildRoseSectionContent(section: Section, lang: string = 'en'): string {
  ...
  if (section.type === 'summary') return `<p class="rounded-xl px-4 py-3 text-sm italic leading-relaxed" style="background-color:${ROSE_50};color:#57534e">${md((c as SummaryContent).text)}</p>`;
  ...
  if (c.items) {
    return `<div class="space-y-2">${c.items.map((it: any) => `<div class="rounded-xl border p-3" style="border-color:${ROSE_100}"><span class="text-sm font-medium" style="color:${PRIMARY}">${esc(it.name || it.title || it.language)}</span>${it.description ? `<p class="text-sm text-zinc-600">${md(it.description)}</p>` : ''}</div>`).join('')}</div>`;
  }
