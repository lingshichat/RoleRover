...
function buildCardSectionContent(section: Section, lang: string = 'en'): string {
  ...
  if (section.type === 'summary') {
    return `<p class="text-sm leading-relaxed text-zinc-600">${md((c as SummaryContent).text)}</p>`;
  }
  ...
  if (c.items) {
    return `<div class="space-y-2">${c.items.map((it: any) => `<div class="rounded-md bg-white p-3" style="border:1px solid #f4f4f5"><span class="text-sm font-medium" style="color:${PRIMARY}">${esc(it.name || it.title || it.language)}</span>${it.description ? `<p class="text-sm text-zinc-600">${md(it.description)}</p>` : ''}</div>`).join('')}</div>`;
  }
  return '';
}
