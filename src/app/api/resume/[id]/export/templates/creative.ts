...
function buildCreativeSectionContent(section: Section, lang: string): string {
  ...
  if (section.type === 'summary') return `<p class="rounded-lg bg-zinc-50 p-4 text-sm leading-relaxed text-zinc-600 italic">${md((c as SummaryContent).text)}</p>`;
  ...
  if (c.items) {
    return `<div class="space-y-2">${c.items.map((it: any) => `<div class="rounded-lg border border-zinc-100 p-3"><span class="text-sm font-medium" style="color:${PRIMARY}">${esc(it.name || it.title || it.language)}</span>${it.description ? `<p class="text-sm text-zinc-600">${md(it.description)}</p>` : ''}</div>`).join('')}</div>`;
  }
  return '';
}
