...
function buildBerlinSectionContent(section: Section, lang: string): string {
  ...
  if (section.type === 'summary') return `<div class="border-l-4 pl-4" style="border-color:${BLUE}"><p class="text-sm leading-relaxed text-zinc-600">${md((c as SummaryContent).text)}</p></div>`;
  ...
  if (c.items) {
    return `<div class="space-y-2">${c.items.map((it: any) => `<div class="border-l-4 pl-4" style="border-color:${BLUE}"><span class="text-sm font-bold" style="color:${TEXT}">${esc(it.name || it.title || it.language)}</span>${it.description ? `<p class="text-sm text-zinc-600">${md(it.description)}</p>` : ''}</div>`).join('')}</div>`;
  }
