...
function buildArtisticSectionContent(section: Section, lang: string): string {
  ...
  if (section.type === 'summary') {
    return `<div class="rounded-lg p-4" style="border:2px dashed ${ACCENT}30;background-color:${PRIMARY}05"><p class="text-sm leading-relaxed text-zinc-600 italic">${md((c as SummaryContent).text)}</p></div>`;
  }
  ...
  if (c.items) {
    return `<div class="space-y-2">${c.items.map((it: any) => `<div class="rounded-lg p-3" style="border:1px dashed ${ACCENT}30"><span class="text-sm font-medium" style="color:${PRIMARY}">${esc(it.name || it.title || it.language)}</span>${it.description ? `<p class="text-sm text-zinc-600">${md(it.description)}</p>` : ''}</div>`).join('')}</div>`;
  }
